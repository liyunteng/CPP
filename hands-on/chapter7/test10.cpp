...
template<typename T>
auto have_start_equal(const T&x, size_t n) -> decltype(T(x) *= n, yes());

no have_start_equal(...);

template<typename T>
auto have_start(const T &x, size_t n) -> decltype(x * n, yes());

no have_start(...);


template<typename T, bool have_start_equal, bool have_start>
struct increase_helper;

template<typename T>
struct increase_helper<T, true, true> {
    static auto f(const T&x, size_t n) {
        std::cout << "T *= n, ignore T * n" << std::endl;
        T y(x);
        return y *= n;
    }
};
...
auto increase(const T&x, size_t n) {
    return increase_helper<T,
        sizeof(have_start_equal(x, n)) == sizeof(yes),
        sizeof(have_start(x, n)) == sizeof(yes)>
        ::f(x, n);
}
